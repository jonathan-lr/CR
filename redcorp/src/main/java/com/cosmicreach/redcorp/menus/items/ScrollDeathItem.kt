...
class ScrollDeathItem(private var econ: Economy, private var balItem: BalanceItem) : AbstractItem() {
    override fun getItemProvider(): ItemProvider {
        val item = CustomItems().DeathScroll(1)
        val meta = item.itemMeta as ItemMeta
        meta.setDisplayName("§3§lDeath Scroll §8§l: §c§l${econ.format(1000.0)}")
    ...
    override fun handleClick(clickType: ClickType, player: Player, event: InventoryClickEvent) {
        ...
        if (econ.getBalance(player) >= 1000.0) {
            ...
            player.inventory.addItem(CustomItems().DeathScroll(1))
