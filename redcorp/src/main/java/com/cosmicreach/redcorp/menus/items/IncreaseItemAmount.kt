...
class IncreaseItemAmount(private val items: List<ShopItem>) : ControlItem<Gui>() {
    private val amount = RedCorp.getPlugin().getPurchaseAmount()
    private val values = listOf(1, 4, 8, 16, 32, 64)

    override fun getItemProvider(gui: Gui): ItemProvider {
        return ItemBuilder(Material.GREEN_STAINED_GLASS_PANE).setDisplayName("§2§lIncrease").setLegacyLore((mutableListOf("§f§lClick to Increase")))
    }
