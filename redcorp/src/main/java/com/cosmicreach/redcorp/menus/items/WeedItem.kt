...
class WeedItem(private var econ: Economy, private var balItem: BalanceItem) : AbstractItem() {
    override fun getItemProvider(): ItemProvider {
        val item = DrugItems().WeedSeed(1)
    ...
    override fun handleClick(clickType: ClickType, player: Player, event: InventoryClickEvent) {
        if (econ.getBalance(player) >= 1000.0) {
            ...
            player.inventory.addItem(DrugItems().WeedSeed(1))
