...
class OnAdvance(private val event : PlayerAdvancementDoneEvent) {
    ...
    fun run() {
        if (event.advancement.display != null) {
            ...
            val forbidden = mutableListOf("Vanilla Tweaks", "Cauldron Concrete", "Custom Nether Portals", "Dragon Drops", "Double Shulker Shells", "More Mob Heads", "Anti Enderman Grief", "Player Head Drops", "Silence Mobs", "Mini Blocks")
