...
class Kyle (private val player: Player, private val kyleConfirm: HashMap<Player, Boolean>, private val kyleStage: HashMap<Player, Int>) {
    fun run () {
        ...
        when (kyleStage[player]) {
            1 -> {
                ...
                val window = Window.single()
                    ...
                    .setGui(Confirm(kyleConfirm).makeGUI())
                    .build()
                window.open()
            }
            2 -> {
                if (kyleConfirm.containsKey(player)) {
                    if(kyleConfirm[player] == true) {
                        kyleConfirm.remove(player)
                        kyleStage[player] = 1
